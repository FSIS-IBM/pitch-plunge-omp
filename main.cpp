...
float dt=0.0002;
int start_timestep = 0, end_timestep = 1000000, writeInterval=654;
float amp_x = 0.0, omega_x = 0.0, amp_y = 0.50, omega_y = 4.0; //body kinematics
float pitch, pitch_m = 0.0, pitch_a = 0.612727, omega_pitch = 4.0, pitch_dot, phase = 1.3264, pitch_axis = -0.50;
float AR = 0.12;
...
int main()
{
    ...
    if(start_timestep == 0)
    {
        ...
        pitch = pitch_m + pitch_a*cos(omega_pitch*0*dt + phase);
        //pitch = z0;
	...
	float u[n_u][m_u], v[n_v][m_v], u_1[n_u][m_u], v_1[n_v][m_v], u_old[n_u][m_u], v_old[n_v][m_v], pressure[n][m], uf1[n][m], vf1[n][m];
...
*/
        ...
        pitch = pitch_m + pitch_a*cos(omega_pitch*k*dt + phase);
        //pitch = z0;
        xc = -pitch_axis*cos(pitch) + x_pivot;
        yc = pitch_axis*sin(pitch) + y_pivot;
        u_pivot = amp_x*omega_x*cos(omega_x*dt*k);
        v_pivot = -amp_y*omega_y*sin(omega_y*dt*k);
        pitch_dot = -pitch_a*omega_pitch*sin(omega_pitch*k*dt + phase);
        //pitch_dot = z1;
